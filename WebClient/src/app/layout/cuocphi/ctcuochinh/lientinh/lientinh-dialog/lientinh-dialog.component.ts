import {  ProvinceApi, DetailProvinceFreightApi } from './../../../../../_shared/bccp-api.services';
...
import { DomesticDetailProvinceFreightFindOneQuery,
  DomesticDomesticFreightRuleFindOneQuery,
  DomesticFreightRuleApi,
  DomesticDetailProvinceFreightCreateCommand,
  DomesticDetailProvinceFreightUpdateCommand
} from 'src/app/_shared/bccp-api.services';

@Component({
  selector: 'app-lientinh-dialog',
  templateUrl: './lientinh-dialog.component.html',
  styleUrls: ['./lientinh-dialog.component.scss']
})
export class LienTinhDialogComponent implements OnInit {
...
async ngOnInit(): Promise<void> {
  ...
  this.isLoading = true;
  if (this.id) {
    const params = new DomesticDetailProvinceFreightFindOneQuery({
      where: { and: [{ id: this.id }] }
    });
    const rs = await this.detailProvinceFreightApi.findOne(params).toPromise();
    if (rs.success) {
...
async submitForm(): Promise<void> {
  ...
  if (this.mode === DialogMode.add) { // them moi
    // add
    const body = new DomesticDetailProvinceFreightCreateCommand(this.myForm.value);
    ...
    const rs = await this.detailProvinceFreightApi.create(body).toPromise();
    if (rs.success) {
    ...
    // edit
    const body = new DomesticDetailProvinceFreightUpdateCommand(this.myForm.value);
    body.id = this.id;
    body.serviceCode = "M";
    body.domesticFreightRuleCode = this.domesticFreightRuleCode;  
    body.domesticFreightStepCode = 'kd';
    body.domesticFreightStepId = this.stepId;
    const rs = await this.detailProvinceFreightApi.update(body).toPromise();
    if (rs.success) {
