...
import { DomesticFarRegionFreightStepApi,
  DomesticDomesticFarRegionFreightStepGetPagingQuery,
   DomesticDomesticFarRegionFreightStepGetPagingQueryResult,
   DomesticDomesticFarRegionFreightStepDeleteCommand } from './../../../_shared/bccp-api.services';
...
@Component({
  selector: 'app-cuocphi-cuocvungxa',
  templateUrl: './cuocvungxa.component.html',
  styleUrls: ['./cuocvungxa.component.scss']
})
export class CuocvungxaComponent implements OnInit {
 ...
 @Input('ruleId') ruleId: number;
  ...
  public listOfData: DomesticDomesticFarRegionFreightStepGetPagingQueryResult[] = [];
  ...
  async getData(paging: PagingModel = { order: { id: false } }): Promise<void> {
    ...
    const where = { and: [] };
    const params = new DomesticDomesticFarRegionFreightStepGetPagingQuery({
      ...paging
    });
    ...
    this.isLoading = true;
    const rs = await this.domesticFarRegionFreightStepApi.getPaging(params).toPromise();
    if (rs.success) {
  ...
  async addDataDialog(): Promise<void> {
    const dialog = this.dialogService.openDialog(option => {
      ...
      option.component = CuocvungxaDialogComponent;
      option.inputs = {
        ruleId: this.ruleId,
        mode: DialogMode.add
      };
  ...
  async deleteData(listId: number[]): Promise<void> {
    ...
    const params = new DomesticDomesticFarRegionFreightStepDeleteCommand({
      id: listId
    });
    const rs = await this.domesticFarRegionFreightStepApi.delete(params).toPromise();
    if (rs.success) {
