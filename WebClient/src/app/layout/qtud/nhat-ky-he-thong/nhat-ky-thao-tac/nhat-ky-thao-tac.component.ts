import { ActionLogApi, LogActionLogGetPagingQuery } from './../../../../_shared/bccp-api.services';
...
@Component({
  selector: 'app-nhat-ky-thao-tac',
  templateUrl: './nhat-ky-thao-tac.component.html',
  styleUrls: ['./nhat-ky-thao-tac.component.scss']
})
export class NhatKyThaoTacComponent implements OnInit {
  ...
  async getData(paging: PagingModel = { order: { CreateDate: false } }): Promise<void> {
    ...
    const where = { and: [] };
    const params = new LogActionLogGetPagingQuery({
      ...paging
    });
    ...
    this.isLoading = true;
    const rs = await this.actionLogApi.getPaging(params).toPromise();
    if (rs.success) {
