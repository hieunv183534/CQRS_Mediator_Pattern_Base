...
import { DimDashboardDatasetApi, DanhMucDashboardDatasetGetPagingQuery, DanhMucDashboardDatasetDeleteCommand } from 'src/app/_shared/bccp-api.services';
import { BieudoDatasetDialogComponent } from './bieudo-dataset-dialog/bieudo-dataset-dialog.component';

@Component({
  selector: 'app-bieudo-dataset',
  templateUrl: './bieudo-dataset.component.html',
  styleUrls: ['./bieudo-dataset.component.scss']
})
export class BieudoDatasetComponent implements OnInit {
  ...
  constructor(
    ...
    private dimDashboardDatasetApi: DimDashboardDatasetApi
  ) { }
  ...
  async getData(paging: PagingModel = { order: { id: false } }): Promise<void> {
    ...
    const where = { and: [] };
    const params = new DanhMucDashboardDatasetGetPagingQuery({
      ...paging
    });
    ...
    this.isLoading = true;
    const rs = await this.dimDashboardDatasetApi.getPaging(params).toPromise();
    if (rs.success) {
  ...
  async addDataDialog(): Promise<void> {
    const dialog = this.dialogService.openDialog(option => {
      ...
      option.component = BieudoDatasetDialogComponent;
      option.inputs = {
        id: null,
        mode: DialogMode.add
      };
  ...
  async deleteData(listId: number[]): Promise<void> {
    ...
    const params = new DanhMucDashboardDatasetDeleteCommand({
      id: listId
    });
    const rs = await this.dimDashboardDatasetApi.delete(params).toPromise();
    if (rs.success) {
