...
namespace NOM.REPORT.Domain.Application.DanhMuc.ReportManagerGroupBy.Commands
{
    public class UpdateCommandHandler : IRequestHandler<UpdateCommand, long>
    {
        ...
        {
            var dataOld = await _context.ReportManagerGroupBy.FindAsync(request.Id);
            if (dataOld == null)
            {
                throw new ValidationException("", "Không tìm thấy dữ liệu cần update");
            }
            var update = _mapper.Map<Dao.Entities.ReportManagerGroupBy>(request);
            //if (update.ColText != dataOld.ColText && await _context.ReportManagerGroupBy.AnyAsync(x => x.ColText == update.ColText))
            //{
